...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "hubert-welsh-model") # Динамический путь для сохранения
...
def main():
    ...
    if " " in vocab_dict:
        vocab_dict["|"] = vocab_dict[" "] # Заменяем пробел на разделитель
        del vocab_dict[" "]
    ...
    dataset.set_transform(prepare_dataset) # Применяем преобразование "на лету"
    ...
    class DataCollatorCTCWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            # Выделяем признаки аудио и метки
            input_features = [{"input_values": feature["input_values"]} for feature in features]
            label_features = [{"input_ids": feature["labels"]} for feature in features]

            # Паддинг (выравнивание) аудио
    ...
    def compute_metrics(pred):
        ...
        pred_str = processor.batch_decode(pred_ids) # Декодируем предсказания
        label_str = processor.batch_decode(pred.label_ids, group_tokens=False) # Декодируем метки
        wer = wer_metric.compute(predictions=pred_str, references=label_str)
        return {"wer": wer}

    # 9. МОДЕЛЬ И ОБУЧЕНИЕ
    print("Загрузка модели HuBERT...")
    ...
    print("Начало обучения HuBERT...")
    trainer.train()
    
    # Сохранение финальной модели и процессора
    trainer.save_model(OUTPUT_DIR)
    processor.save_pretrained(OUTPUT_DIR)
    print(f"✅ Готово! Модель HuBERT сохранена в {OUTPUT_DIR}")
